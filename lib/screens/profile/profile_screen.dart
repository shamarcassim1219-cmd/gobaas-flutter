...
import '../wallet/top_up_withdraw_history_screen.dart';
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _changeLanguage() async {
    ...
    await locale.setLanguageFromSettings(chosen);
  }
  ...
  Future<void> _addEmail() async {
    ...
    try {
      await AuthService.instance.addEmail(email);
      if (!mounted) return;
      await _loadUser();
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Email added.')),
      );
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Unable to add email. Please try again.')),
      );
    }
  }
          ...
          children: [
            ...
            _sectionCard([
              ...
              email.isNotEmpty
                  ...
                  : _actionRow(Icons.email_outlined, 'Add Email', _addEmail),
            ]),
            const SizedBox(height: 16),
            _sectionCard([
              ...
              _actionRow(Icons.flag_outlined, 'My Complaints', () {
              ...
              _actionRow(Icons.card_giftcard_outlined, 'Earn with GOBAAS', () {
                if (mobile.isEmpty && email.isEmpty) {
              ...
              _actionRow(Icons.history, 'Top Up / Withdraw History', () {
                Navigator.of(context).push(MaterialPageRoute(builder: (_) => const TopUpWithdrawHistoryScreen()));
              }),
  ...
  /// Fixed to wrap both label and value in Flexible/Expanded - a
  /// fixed Row without this overflows once the Sinhala/Tamil label
  /// text is longer than the English original was.
