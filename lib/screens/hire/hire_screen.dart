...
class _HireScreenState extends State<HireScreen> {
  ...
  /// Payment didn't go through - the order still exists (the
  /// backend has no order-cancellation endpoint to undo that part),
  /// but rather than dropping the customer into a generic "order
  /// created" screen, this keeps them in the hire flow with a clear
  /// choice: try paying again on the same order, or leave it
  /// pending and finish later from My Orders.
  Future<void> _showPaymentFailedDialog(MybaasOrder order, String reason) async {
    final retry = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('$reason\n\nYour order is saved but still unpaid.'),
        actions: [
          TextButton(
    ...
    if (retry == true) {
      ...
      final t = context.read<LocaleController>().t;
      _showOrderCreatedDialog(t, payNow: true, paymentSucceeded: false);
    }
  }
