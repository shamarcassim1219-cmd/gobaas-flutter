...
class _HireScreenState extends State<HireScreen> {
  ...
  Future<void> _submit() async {
    if (!_canSubmit || !_agreedToTerms) return;

    final locale = context.read<LocaleController>();
    final t = locale.t;

    // Fire-and-forget - records the agreement and emails a copy of
    // the terms if this account has an email on file, but never
    // blocks placing the order itself.
    LegalService.instance.agree(context: 'customer_order', language: locale.langCode);
    ...
    try {
      ...
      if (_paymentMethod == 'pay_now') {
        ...
        _showOrderCreatedDialog(t, payNow: false, paymentSucceeded: false);
      }
  ...
  /// After a pay_now order is created, let the customer settle it
  /// immediately - from wallet balance, or by card through PayHere.
  ...
  Future<void> _payWithWallet(MybaasOrder order) async {
    ...
    try {
      ...
      if (!mounted) return;
      _showOrderCreatedDialog(t, payNow: true, paymentSucceeded: true);
    } on ApiException catch (e) {
      if (!mounted) return;
      await _showPaymentFailedDialog(order, e.message);
    } catch (e) {
      if (!mounted) return;
      await _showPaymentFailedDialog(order, 'Unable to pay from wallet.');
    }
  }
  ...
  Future<void> _payWithCard(MybaasOrder order) async {
    try {
      ...
      if (completed == true) {
        ...
        await _showPaymentFailedDialog(order, 'Card payment was not completed.');
      }
    } catch (e) {
      if (!mounted) return;
      await _showPaymentFailedDialog(order, 'Unable to start card payment.');
    }
  }

  /// Confirmed against the user's requested behavior: rather than
  /// leaving an unpaid order sitting in My Orders, a failed/abandoned
  /// pay_now payment cancels the order outright - the customer just
  /// retries the payment on the SAME still-open order, or backs out
  /// to a clean slate instead of a dangling unpaid one.
  Future<void> _showPaymentFailedDialog(MybaasOrder order, String reason) async {
    ...
    // Declined to retry - cancel the order rather than leaving it
    // unpaid, then let them try hiring again from a clean state.
    try {
      await OrdersService.instance.cancelOrder(order.id);
    } catch (_) {
      // Cancellation failing silently is fine here - the order just
      // stays unpaid in My Orders as a fallback, no worse than before.
    }
  ...
  void _showOrderCreatedDialog(String Function(String) t, {required bool payNow, required bool paymentSucceeded}) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: (_canSubmit && _agreedToTerms && !_submitting) ? _submit : null,
                child: _submitting
                    ? const SizedBox(
                        height: 22,
                        width: 22,
