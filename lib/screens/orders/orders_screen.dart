...
class _OrdersScreenState extends State<OrdersScreen> with WidgetsBindingObserver {
  ...
  // Loaded from the backend's own records (not just tracked for
  // this session) so an order that was already reviewed or already
  // complained about stays hidden from those options even after the
  // app is closed and reopened.
  Set<String> _reviewedOrderIds = {};
  Set<String> _complainedOrderIds = {};
  ...
  /// Refreshes the order list whenever the app comes back to the
  /// foreground - without this, an order status change (accepted,
  /// completed) made from the Baas's side wouldn't show up here
  /// until manually pulled to refresh.
  @override
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final results = await Future.wait([
        OrdersService.instance.myOrders(),
        ReviewService.instance.myReviews(),
        ComplaintService.instance.myComplaints(),
      ]);

      final orders = results[0] as List<MybaasOrder>;
      final reviews = results[1] as List<MyReview>;
      final complaints = results[2] as List<Complaint>;

      if (!mounted) return;
      setState(() {
        _orders = orders;
        _reviewedOrderIds = reviews.map((r) => r.orderId).toSet();
        _complainedOrderIds = complaints.map((c) => c.orderId).toSet();
  ...
  Future<void> _showCompletedOrderOptions(MybaasOrder order, bool alreadyReviewed, bool alreadyComplained) async {
    ...
    if (choice == 'review') {
      ...
      Navigator.of(context).push(MaterialPageRoute(builder: (_) => ComplaintScreen(order: order))).then((_) => _load());
    }
  }
  ...
  Widget _buildOrderCard(MybaasOrder order) {
    ...
    final alreadyComplained = _complainedOrderIds.contains(order.id);
    final hasMoreOptions = !alreadyReviewed || !alreadyComplained;
        ...
        child: Column(
          ...
          children: [
            ...
            if (isCompleted) ...[
              const SizedBox(height: 10),
              InkWell(
                onTap: hasMoreOptions ? () => _showCompletedOrderOptions(order, alreadyReviewed, alreadyComplained) : null,
                    ...
                    children: [
                      Icon(
                        ...
                        color: !hasMoreOptions
                            ? AppColors.success
                            : (alreadyReviewed ? AppColors.success : AppColors.primary),
                      ),
                      const SizedBox(width: 4),
                      Text(
                        _completedOrderStatusText(alreadyReviewed, alreadyComplained),
                        style: TextStyle(
                          fontSize: 12,
  ...
  String _completedOrderStatusText(bool alreadyReviewed, bool alreadyComplained) {
    if (alreadyReviewed && alreadyComplained) return 'Reviewed and reported';
    if (alreadyReviewed) return 'Reviewed - tap to report a problem';
    if (alreadyComplained) return 'Reported - tap to leave a review';
    return 'Tap to review or report a problem';
  }
