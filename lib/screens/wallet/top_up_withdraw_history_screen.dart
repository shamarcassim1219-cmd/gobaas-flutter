...
class _TopUpWithdrawHistoryScreenState extends State<TopUpWithdrawHistoryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: Text(t('topUpWithdrawHistory'))),
      body: SafeArea(
        child: _loading
                ...
                child: _entries.isEmpty && _error == null
                    ...
                    : ListView(
                        ...
                        children: [
                          ...
                          ..._entries.map((e) => _buildEntryRow(e, t)),
                        ],
                      ),
              ),
      ),
          ...
          child: Center(
            child: Column(
              children: [
                ...
                Text(t('noTopUpsWithdrawalsYet'), style: const TextStyle(color: AppColors.textMuted)),
              ],
            ),
          ),
        ),
  ...
  Widget _buildEntryRow(LedgerEntry entry, String Function(String) t) {
    final isTopUp = entry.type == 'TOPUP';
    final isPending = entry.state == 'pending' || entry.state == 'withdrawing';

    return InkWell(
      onTap: () => _showEntryDetail(entry, isTopUp, t),
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    isTopUp ? t('topUpLabel') : t('withdrawalLabel'),
                  ...
                  if (isPending) ...[
                    ...
                    Container(
                      ...
                      child: Text(
                        entry.state == 'withdrawing' ? t('processing') : t('pending'),
  ...
  void _showEntryDetail(LedgerEntry entry, bool isTopUp, String Function(String) t) {
    final isPending = entry.state == 'pending' || entry.state == 'withdrawing';
      ...
      builder: (context) => SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(isTopUp ? t('topUpLabel') : t('withdrawalLabel'), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
              const SizedBox(height: 16),
              _detailRow(t('total'), '${entry.amount < 0 ? '-' : '+'}${formatMoney(entry.amount.abs(), isInternational: _isInternational)}'),
              _detailRow(t('dateAndTime'), timeLabel),
              _detailRow(t('status'), isPending ? (entry.state == 'withdrawing' ? t('processing') : t('pending')) : t('completedStatus')),
              if (entry.note.isNotEmpty) _detailRow(t('note'), entry.note),
