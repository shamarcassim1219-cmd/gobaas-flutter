...
class _StartupGateState extends State<_StartupGate> {
  ...
  Future<void> _prepareSession() async {
    try {
      ...
      // A token can only be registered once an authenticated session
      // exists (guest or real) - fire-and-forget, since a failed
      // registration shouldn't block startup.
...
/// Guards an action that needs a real (non-guest) account - checks
/// the current session, and if it's a guest, shows the same
/// confirm-then-login dialog used throughout the app (matching
/// LiveChatScreen's pattern) instead of letting the action run and
/// fail against the backend. Returns true if the account is real
/// and the caller should proceed; false if a guest was shown the
/// dialog (whether or not they chose to log in).
Future<bool> requireRealAccount(BuildContext context) async {
  final user = await ApiClient.instance.getUser();
  final mobile = user?['mobile'] as String? ?? '';
  final email = user?['email'] as String? ?? '';
  final isGuest = mobile.isEmpty && email.isEmpty;
