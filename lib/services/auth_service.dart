...
class AuthService {
  ...
  Future<Map<String, dynamic>> verifyRegisterOtp(String mobile, String otp, {String? role, String? referralCode}) async {
    final data = await _api.post(
  ...
  /// Confirmed against the real backend - same role-agnostic
  /// endpoints the Baas app's SecurityScreen uses. A two-step OTP
  /// flow: request a code to the NEW address/number, then verify it
  /// to actually apply the change. Neither takes effect until the
  /// OTP is verified.
