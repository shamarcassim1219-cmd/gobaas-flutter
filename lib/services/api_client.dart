...
class ApiClient {
  ...
  /// A guest account (created via /api/auth/guest-register) has
  /// neither a mobile nor an email on it - used to gate features
  /// that need a real, verified identity (search, wallet top-up/
  /// withdraw, referrals) behind login, the same way HireScreen's
  /// MOBILE_REQUIRED handling does for order creation.
  ...
  Future<Map<String, String>> _headers({bool auth = true}) async {
    ...
    // Sent on every request, authenticated or not, so a banned
    // device is blocked at login/register (before any token exists)
    // just as much as on every later authenticated call.
    try {
      ...
      // Device ID unavailable - proceed without it rather than
