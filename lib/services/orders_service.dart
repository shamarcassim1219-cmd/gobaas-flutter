...
class OrdersService {
  ...
  /// fetches a single order fresh (scoped to the authenticated
  /// customer's own orders), used by OrderDetailScreen so every
  /// screen that links to an order (reviews, complaints, wallet
  /// history) always shows current data rather than a stale copy
  /// carried over from whatever list it was tapped from.
  Future<MybaasOrder> getOrder(String orderId) async {
